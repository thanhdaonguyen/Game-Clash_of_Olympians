...
bool loadMedia() {
    ...
    if(!gFlydemon.loadFromFile(gRenderer,"assets/flyingdemon.png")) {
        printf ("Failed to load flydemon image");
        success = false;
    }
...
int main( int argc, char* args[] ) {
    ...
    if( !init() ) {
        ...
        if( !loadMedia() ) {
            ...
            Enemy fly1(1590,250,"Flydemon");
            Enemy fly2(1700,300,"Flydemon");
            ...
            //Gamemode
            enum { END_GAME, RESTART, QUIT_QAME, IGNORE };
            int choice = IGNORE ;
            ...
            while( !quit ) {
                ...
                for (unsigned long i = 0; i < emVec.size(); i++) {
                    if (emVec[i]->mType == "Goblin") {
                        emVec[i]->render(gRenderer, gGoblin);
                    }
                    if (emVec[i]->mType == "Flydemon") {
                        emVec[i]->render(gRenderer, gFlydemon);
                    }
                }
                ...
                if (HP < 0) {
                    ...
                    gScore.render(gRenderer, (SCREEN_WIDTH/2 - gScore.getWidth()/2), (SCREEN_HEIGHT/2 - gScore.getHeight()/2));
                    SDL_RenderPresent(gRenderer);
                    SDL_Delay(3000);
